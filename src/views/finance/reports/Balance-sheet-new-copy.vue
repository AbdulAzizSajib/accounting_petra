<template>
  <MainLayout>
    ...
    <div class="balance-sheet-header bg-[#004d8c] text-white px-4 py-2">
      <div class="flex items-center justify-between">
        <h1 class="text-lg font-bold">Balance Sheet</h1>
        <div class="flex items-center gap-4">
          <div class="flex items-center gap-2">
            ...
            <input
              ...
              v-model="DateFrom"
              :max="DateTo"
              class="px-2 py-1 text-black text-sm rounded border-0"
            />
          </div>
          <div class="flex items-center gap-2">
          ...
          <a-button
            ...
            :loading="loading"
            :disabled="dateError"
            class="!bg-white !text-[#004d8c] !border-0 hover:!bg-gray-100"
          >
            Preview
          </a-button>
          <button
    ...
    <!-- Company Header -->
    <div class="bg-[#004d8c] text-white text-center py-1 border-t border-[#0066aa]">
      <span class="font-semibold">{{ companyName }}</span>
    </div>

    <!-- Loading State -->
    <div v-if="loading" class="flex justify-center items-center py-20 bg-[#e8f4fc]">
      <a-spin size="large" />
    </div>
    ...
    <div
      ...
      class="flex flex-col items-center justify-center py-20 bg-[#e8f4fc]"
    >
    ...
    <div v-else-if="hasData" class="balance-sheet-container bg-[#e8f4fc] min-h-[calc(100vh-200px)] flex flex-col">
      <!-- Main Content Area -->
      <div class="flex flex-1">
        <!-- Left Side - Liabilities -->
        <div class="w-1/2 border-r border-[#00000033] flex flex-col">
          <!-- Column Header -->
          <div class="flex bg-[#d0e8f5] border-b border-[#00000033]">
            <div class="flex-1 px-3 py-2 font-bold text-[#00264d]">Liabilities</div>
            <div class="w-36 px-3 py-2 text-right text-xs text-gray-600">
              {{ companyName }}<br />
              <span class="text-[10px]">as at {{ formatDisplayDate(DateTo) }}</span>
            </div>
          </div>
          ...
          <div class="px-1 flex-1">
            ...
            <div class="bg-[#ffcc00] px-3 py-1 font-semibold text-sm mt-1">
              Capital Account
            </div>
            <template v-if="f_balanceSheetData?.length > 0">
              <div
                ...
                :key="`f-${index}`"
                class="flex justify-between px-3 py-0.5 text-sm hover:bg-[#d0e8f5]"
              >
                <span>{{ item?.ACType1Details }}</span>
                <span>{{ formatAmount(item?.Amount) }}</span>
              </div>
            </template>
            <div class="flex justify-between px-3 py-0.5 text-sm font-semibold">
              <span></span>
              <span>{{ formatAmount(f_total) }}</span>
            </div>

            <!-- Non-Current Liabilities -->
            <div class="px-3 py-1 font-semibold text-sm mt-2">
              Loans (Liability)
            </div>
            ...
            <div class="flex justify-between px-3 py-0.5 text-sm font-semibold">
              <span></span>
              <span>{{ formatAmount(e_total) }}</span>
            </div>
            ...
            <div class="flex justify-between px-3 py-0.5 text-sm font-semibold">
              <span></span>
              <span>{{ formatAmount(d_total) }}</span>
            </div>
            ...
            <div class="flex justify-between px-6 py-0.5 text-sm">
              <span>Opening Balance</span>
              <span>{{ formatAmount(openingBalance) }}</span>
            </div>
            <div class="flex justify-between px-6 py-0.5 text-sm">
              <span>Current Period</span>
              <span>{{ formatAmount(currentPeriodPL) }}</span>
            </div>

            <!-- Difference in Opening Balance -->
            <div class="flex justify-between px-3 py-1 text-sm mt-2">
              <span>Difference in opening balances</span>
              <span>{{ formatAmount(differenceInOpening) }}</span>
            </div>
          </div>
        </div>
        ...
        <div class="w-1/2 flex flex-col">
          <!-- Column Header -->
          <div class="flex bg-[#d0e8f5] border-b border-[#00000033]">
            <div class="flex-1 px-3 py-2 font-bold text-[#00264d]">Assets</div>
            <div class="w-36 px-3 py-2 text-right text-xs text-gray-600">
              {{ companyName }}<br />
              <span class="text-[10px]">as at {{ formatDisplayDate(DateTo) }}</span>
            </div>
          ...
          <!-- Assets Content -->
          <div class="px-1 flex-1">
            <!-- Current Liabilities shown as first item on Assets side (as per image) -->
            <div class="bg-[#ffcc00] px-3 py-1 font-semibold text-sm mt-1">
              Current Liabilities
            </div>
            <div class="flex justify-between px-3 py-0.5 text-sm font-semibold">
            ...
            <div class="flex justify-between px-3 py-0.5 text-sm font-semibold">
              <span></span>
              <span>{{ formatAmount(o_total) }}</span>
            </div>
            ...
            <div class="flex justify-between px-3 py-0.5 text-sm font-semibold">
              <span></span>
              <span>{{ formatAmount(b_total) }}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div class="flex border-t border-[#00000033] bg-[#d0e8f5]">
        <!-- Total Liabilities -->
        <div class="w-1/2 flex justify-between px-3 py-2 font-bold text-sm border-r border-[#00000033]">
          <span>Total</span>
          <span>{{ formatAmount(totalLiabilities) }}</span>
        </div>
        <!-- Total Assets -->
        <div class="w-1/2 flex justify-between px-3 py-2 font-bold text-sm">
          <span>Total</span>
    ...
    <div v-else class="flex flex-col items-center justify-center py-20 bg-[#e8f4fc] min-h-[calc(100vh-200px)]">
      <Icon
